...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['verify_otp'])) {
    ...
    if (isset($_SESSION['phone_otp']) && $otp == $_SESSION['phone_otp']) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['generate_otp'])) {
    ...
    if (empty($phone)) {
        $error_message = "Phone number is required to send OTP.";
    } else {
        // Generate a 4-digit OTP
        $otp = rand(1000, 9999);
        $_SESSION['phone_otp'] = $otp;
        
        // In a real implementation, you would integrate with an SMS API here
        // For example, using Twilio or any other SMS gateway service
        
        // For development purposes, we'll just show the OTP
        $otp_generated = true;
        $otp_success_message = "OTP has been sent to your phone number.";
        $dev_otp_message = "Development mode: Your OTP is $otp";
        
        // Example of how SMS integration might look (commented out)
        /*
        ...
        try {
            $message = $client->messages->create(
                ...
                [
                    'from' => $twilio_number,
                    'body' => "Your OTP for bike rental booking is: $otp"
                ]
            );
            ...
            $error_message = "Failed to send OTP. Error: " . $e->getMessage();
...
<script>
    // Move to next OTP input box when digit is entered
    ...
    // Make OTP container visible if it was previously shown
    ...
    // Address suggestions function (placeholder)
    function fetchAddress(query) {
        // This would normally call an API for address suggestions
